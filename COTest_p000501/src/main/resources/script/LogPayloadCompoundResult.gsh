	...
	if(enableLogging != null && enableLogging.toUpperCase().equals("TRUE")){
		...
		if(messageLog != null){
			if(processVariant != null && processVariant.equals("1")){
				messageLog.addAttachmentAsString("Compound API Result (Last Modified Date)", body, "text/xml");
			}else if(processVariant != null && processVariant.equals("2")){
				messageLog.addAttachmentAsString("Compound API Result (Monitor)", body, "text/xml");
			}else{
