...
def Message processData(Message message) {
    ...
    if(messageLog != null){
    	...
    	messageLog.setStringProperty("COMPANY_TERRITORY_CODE ", companyTerritoryCode);
    	messageLog.setStringProperty("EMPLOYEE_CLASS ", employeeClass);
    	messageLog.setStringProperty("CONTINGENT_WORKERS ", contingentWorkers);
    	messageLog.setStringProperty("PERSON_ID_EXTERNAL ", personIdExternal);
    	messageLog.setStringProperty("USER_ID ", userId);
    	messageLog.setStringProperty("EXTERNAL_COST_CENTER_ID_USAGE ", externalCostCenterIDUsage);
    	messageLog.setStringProperty("MULTIPLE_JOB_EVENTS ", multipleJobEvents);
    	messageLog.setStringProperty("ENABLE_NOTIFICATION ", enableNotification);
    	messageLog.setStringProperty("CUSTOM_MDF_OBJECTS ", customMDFObjects);
    	messageLog.setStringProperty("PROCESS_VARIANT ", processVariant);
    	messageLog.setStringProperty("USER_SET_LAST_MODIFIED_DATE_TIME ", userSetLastModifiedDateTime);
    	messageLog.setStringProperty("SFSF_EC_PUSH_URL ", pushURL);
    }
    ...
    if(externalCostCenterIDUsage == null || (!externalCostCenterIDUsage.equals("TRUE") && !externalCostCenterIDUsage.equals("FALSE"))){
    	throw new Exception("Configuration Error: Please enter either TRUE or FALSE in the parameter EXTERNAL_COST_CENTER_ID_USAGE.   ");
    }
    if(multipleJobEvents == null || (!multipleJobEvents.equals("TRUE") && !multipleJobEvents.equals("FALSE"))){
    	throw new Exception("Configuration Error: Please enter either TRUE or FALSE in the parameter MULTIPLE_JOB_EVENTS.   ");
    }
    if(enableNotification == null || (!enableNotification.equals("TRUE") && !enableNotification.equals("FALSE"))){
    	throw new Exception("Configuration Error: Please enter either TRUE or FALSE in the parameter ENABLE_NOTIFICATION.   ");
    }
    
    //Set general object list for select statement
    String objectsSelectStatement = " personal_documents_information, dependent_information, person, personal_information, address_information, phone_information, email_information, employment_information, job_information, compensation_information, paycompensation_recurring, paycompensation_non_recurring, payment_information, direct_deposit, national_id_card, alternative_cost_distribution, deduction_recurring, deduction_non_recurring, accompanying_dependent, global_assignment_information, ItDeclaration, job_relation, person_relation ";
	...
	if(enableNotification.toUpperCase().equals("TRUE")) {
		SFAPIParameters = SFAPIParameters + ";triggerReplicationMonitorUpdate=yes";
	}
